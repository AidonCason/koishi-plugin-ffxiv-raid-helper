...
export const getTeamInfo = async (ctx: Context, teams: TeamListTable[]) => {
  ...
  return teams.map(
    team =>
      `${team.team_region} - [${team.group_name}] ${team.team_name} 时间： ${team.raid_start_time.toLocaleString(locale_settings.current, date_locale_options)} 报名人数： ${sign_ups.find(d => d.team_name == `${team.id}`)?.count ?? 0}/${team.max_members}`
  );
};
