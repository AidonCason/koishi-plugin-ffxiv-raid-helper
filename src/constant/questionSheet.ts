...
export const getSheet = (server_name: string, config: Config) => {
    const questions: ReadonlyArray<QuestionDefine> = [
        ...
        },
        {
            ...
            name: "所在服务器",
            content: '所在服务器',
            construct_range: _ => new Map(config.server_name_map[server_name].map((server, idx) => [(idx + 1).toString(), server])),
        },
        {
            label: '4',
        ...
        },
        {
            ...
            type: QuestionType.Boolean,
            name: "是否接受调剂",
            content: '选择职业前请您注意，我们保证所有的开荒位置先到先得，但是如果您所选择的职能已经满员了，您是否愿意被调剂到其他职能？',
            answer_range: [1, 0]
        },
        {
            label: '7',
            ...
            name: "主选职能",
            content: '职能-职业选择:职能',
            construct_range: _ => new Map(Object.keys(duties).map((duty, idx) => [(idx + 1).toString(), duty])),
        },
        {
            label: '7-1',
            type: QuestionType.SignleChoice,
            ...
            construct_range: input => {
                ...
                return new Map(duties[duty].map((job, idx) => [(idx + 1).toString(), job]));
            }
        },
        {
            label: '8',
