...
export const getSheet = (server_name: string, config: Config) => {
    const questions: ReadonlyArray<QuestionDefine> = [
        ...
        },
        {
            ...
            name: '所在服务器',
            content: '所在服务器',
            construct_range: () => new Map(config.server_name_map[server_name].map((server, idx) => [(idx + 1).toString(), server])),
        },
        {
            label: '4',
        ...
        },
        {
            ...
            name: '主选职能',
            content: '职能-职业选择:职能',
            construct_range: () => new Map(Object.keys(duties).map((duty, idx) => [(idx + 1).toString(), duty])),
        },
        {
            label: '7-1',
            type: QuestionType.SignleChoice,
